...
import com.kumbh.mimo.config.auth.dto.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String registrationId = userRequest.getClientRegistration().getRegistrationId();            //현재 로그인 진행 중인 서비스를 구분하는 코드(구글,네이버,카카오 구분)
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails()     //PK 의미 필드값. 궁금하면 좀 더 찾아보기.
    ...
    private User saveOrUpdate(OAuthAttributes attributes) {                 //사용자 정보가 업데이트 됐을 때(이름, 프로필 사진) User 엔티티에도 반영
